...
class ImageExporter():
    ...
    def export(self, collage_np, batch_id, global_id, img_parameters, vertices, faces):
        ...
        imageio.imsave(os.path.join(self.output_dir, "{}.jpg".format(global_id)), collage_np)

        with open(os.path.join(self.output_dir, "{0:04}.pkl".format(global_id)), 'wb') as f:
            pkl.dump(img_parameters, f)

def main():
    OUTPUT_DIR = os.path.join("/data/shihao/badja/exported", config.CHECKPOINT_NAME, config.EPOCH_NAME)
    ...
    model.load_checkpoint(os.path.join("/data/shihao/badja/checkpoints", config.CHECKPOINT_NAME), config.EPOCH_NAME)
